...
import com.qartf.popularmovies.domain.mapper.asDomainModel
...
class RepositoryImpl(
    ...
    override suspend fun getMoviesAsync(sortBy: String, pageNo: String): MovieContainer {
    ...
    override suspend fun getMovieReviewsAsync(movieId: String): ReviewContainer {
        val args = HashMap<String, String>()
        args[API_KEY] = THE_MOVIE_DB_API_TOKEN
    ...
    override suspend fun getMovieTrailersAsync(movieId: String): VideoContainer {
        val args = HashMap<String, String>()
        args[API_KEY] = THE_MOVIE_DB_API_TOKEN
    ...
    @MainThread
    override fun getMoviesPaging(sortBy: String, genre: String): Listing<Movie> {
        val args = HashMap<String, String>()
        args[API_KEY] = THE_MOVIE_DB_API_TOKEN
        args[SORT_BY] = sortBy
    ...
    @MainThread
    override fun getMoviesPagingDB(sortBy: String, pageSize: Int): Listing<Movie> {
    ...
    override fun getMovieWithId(movieItemId: String): LiveData<MovieItem?> {
        return productDatabase.getMovieWithId(movieItemId)
    }

    override suspend fun insert(movieItem: MovieItem) {
        withContext(Dispatchers.IO) {
            productDatabase.insert(movieItem)
        }
    }

    override suspend fun delete(movieItemId: String) {
        withContext(Dispatchers.IO) {
